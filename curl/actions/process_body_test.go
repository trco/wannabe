...
func TestProcessBody(t *testing.T) {
	...
	for testKey, tc := range testCases {
		processedBody, _ := ProcessBody(tc.Body, tc.Config)

		if testKey == "emptyBody" && !(len(tc.Body) == 0 && len(processedBody) == 0) {
			t.Errorf("Failed test case: %v, Expected: %v, Actual: %v", testKey, tc.Expected, processedBody)
		}

		if testKey != "emptyHeadersMap" && !reflect.DeepEqual(tc.Expected, processedBody) {
			t.Errorf("Failed test case: %v, Expected: %v, Actual: %v", testKey, tc.Expected, processedBody)
		}
	}
}
